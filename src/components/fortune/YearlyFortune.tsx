...
function buildYearlyContext(
  ...
  // 流年基础信息
  lines.push('## 流年基础')
  lines.push(`- 流年：${year}年（${yearly.heavenlyStem || '未知'}${yearly.earthlyBranch || '未知'}年）`)
  lines.push(`- 流年四化：${Array.isArray(yearly.mutagen) ? yearly.mutagen.join('、') : '无'}`)
  lines.push(`- 流年命宫位置：${Array.isArray(yearly.palaceNames) ? yearly.palaceNames[0] : '未知'}`)
  lines.push('')

  // 大限信息
  lines.push('## 当前大限')
  lines.push(`- 大限天干：${decadal.heavenlyStem || '未知'}`)
  lines.push(`- 大限四化：${Array.isArray(decadal.mutagen) ? decadal.mutagen.join('、') : '无'}`)
  lines.push(`- 大限命宫位置：${Array.isArray(decadal.palaceNames) ? decadal.palaceNames[0] : '未知'}`)
  lines.push('')
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      ...
      if (!horoscope.yearly || !horoscope.decadal) {
        throw new Error('无法获取流年运限数据，请检查出生信息是否正确')
      }
